...
#define STU_MAX_OPEN 3
#define STU_DISCOUNT 0.8f
...
int stu_ctl(struct node **users, struct bookslinklist *books, struct node **orders)
{
    ...
    if (prompt_login(users, UT_STU, &puser_node) != 0)
    {
        return -1;
    }
    ...
    while (1)
    {
        ...
        switch (cmd)
        {
            ...
            case 4:
                /* 租书 */
                create_order_flow(users, books, orders, puser, STU_DISCOUNT, STU_MAX_OPEN);
                pause_screen();
                break;
